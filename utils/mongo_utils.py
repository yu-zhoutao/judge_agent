...
if __name__ == "__main__":
    ...
    # 插入文档
    doc_id = mongo.insert_one("test_collection", {"name": "测试", "value": 123})
    print(f"插入文档ID: {doc_id}")

    # 查询文档
    doc = mongo.find_one("test_collection", {"name": "测试"})
    print(f"查询结果: {doc}")

    # 更新文档
    mongo.update_one("test_collection", {"name": "测试"}, {"$set": {"value": 456}})

    # 查询所有文档
    docs = mongo.find_many("test_collection", {})
    print(f"所有文档: {docs}")

    # 删除文档
    mongo.delete_one("test_collection", {"name": "测试"})
    ...
    async def async_example():
        print("\n=== 异步MongoDB使用示例 ===")
        async_mongo = get_async_mongo()

        # 插入文档
        doc_id = await async_mongo.insert_one("test_collection", {"name": "异步测试", "value": 789})
        print(f"插入文档ID: {doc_id}")

        # 查询文档
        doc = await async_mongo.find_one("test_collection", {"name": "异步测试"})
        print(f"查询结果: {doc}")

        # 更新文档
        await async_mongo.update_one("test_collection", {"name": "异步测试"}, {"$set": {"value": 999}})

        # 查询所有文档
        docs = await async_mongo.find_many("test_collection", {})
        print(f"所有文档: {docs}")

        # 删除文档
        await async_mongo.delete_one("test_collection", {"name": "异步测试"})
